...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    apple3.changeColor(newColor = "golden")
    ...
    val fruitBasket = List(
     ...
     //println(fruitBasket)
    val f = fruitBasket.find(_.name=="apple")
    println(f)
   // println(("Apple = "  + f.name + " : " + f.color))
    ...
    def getOrange(basket: List[Fruit]): List[Fruit] = for (fruit <- basket if fruit.name =="orange") yield  fruit
    def getKiwis(basket: List[Fruit]): List[Fruit] = for (fruit <- basket if fruit.name =="kiwi") yield  fruit
